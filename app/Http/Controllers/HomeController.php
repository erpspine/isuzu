...
$master['today_repair_float']=round(($today_repair_float),2);
    ...
    public function loadpeople_dashboard()
    {
...
return view('home.partials.people')->with(compact('master'));


    }
    public function loadquality_dashboard()
    {
        //OFFLINE
        $today = Carbon::yesterday()->format('Y-m-d');
        $first = Carbon::create($today)->startOfMonth()->format('Y-m-d');
...
$master['lcvwdpvtarget'] = (getGCATarget($today) == '0') ? 0 : round(getGCATarget($today)->lcvwdpv,1);
    ...
    return view('home.partials.quality')->with(compact('master'));


    }
    public function loadresponsiveness_dashboard()
    {
        //OFFLINE
...
$master['efftag'] = round($efftag,0);
        ...
        return view('home.partials.responsiveness')->with(compact('master'));


    }

    public function loadattendance_dashboard()
    {
        $yesterday = carbon::yesterday()->format('Y-m-d');
        //UNLOGGED ATTENDANCE
$allschdates = Production_target::whereBetween('date', ["2022-01-01", $yesterday])
->groupby('date')->get(['date']);
$efshops = Shop::where('check_shop','=','1')->get();



$attendances= Attendance::whereIn('date',$allschdates)->whereHas('shop', function ($query) {
    $query->where('check_shop','1');
})->groupBy('shop_id','date')->get(['shop_id','date'])->toArray();



$count_unlogged=array();
foreach($allschdates as $adate){
    $date=$adate->date;

    foreach($efshops as $shop){
        $shop_id=$shop->id;
        $count_unlogged[]=array('shop_id'=>$shop_id,
                         'date'=> $date);
...
$unlogged=count($count_unlogged)-count($attendances);
...
$master['delayed'] = Unitmovement::where([['datetime_in','!=',$today],['current_shop','>',0]])->count();
        ...
        return view('home.partials.attendance')->with(compact('master'));
