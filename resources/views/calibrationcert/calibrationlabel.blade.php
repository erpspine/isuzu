...
<head>
    ...
    <title>Print TTSM Certificate</title>
...
@foreach($unit_details as $details)
    @while($details['qty'] > 0)
        ...
        <div style="display:inline-block;vertical-align:middle;line-height:16px !important;">
            {{-- Business Name --}}
            <br/>
            <b style="display: block !important;font-size:18px;" class="text-uppercase">CALIBRATION CERTIFICATE </b><br/><br/><br/><br/>
                <i>Torque Type</i><b style="display: block !important;font-size:20px;" class="text-uppercase"> {{$details['details']['torque_type']}} </b><br/><br/>
                <i>Tool ID</i><b style="display: block !important;font-size:20px;" class="text-uppercase">{{$details['details']['tool_id']}}</b><br/><br/>
                <i>Next Calibration Date</i> <b style="display: block !important;font-size:20px;" class="text-uppercase">{{$details['details']['cdate']}}</b><br/><br/><br/><br/>
                <b style="display: block !important;font-size:60px;" class="text-uppercase">{{$details['details']['status']}}</b><br/><br/>
            

            <br>
            {{-- Barcode --}}
            <div style="padding-bottom:5px;"><img class="center-block" style="max-width:90%; !important;height:20px in !important;" src="data:image/png;base64,{{DNS1D::getBarcodePNG($details['details']['sku'], 'C128', 3,30,array(39, 48, 54))}}">
...
@endif
 ...
 <style>  .text-center{
    ...
    .sticker-border{
        border: 0.2px dotted grey !important;
        overflow: hidden;
