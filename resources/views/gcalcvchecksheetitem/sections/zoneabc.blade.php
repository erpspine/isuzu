        ...
        @if (count($records) > 0)
            <tbody>
                ...
                @foreach ($records as $record)
                    <tr>
                        <td><input name="item_id[{{ $i }}]" type="hidden" value="{{ $record->id }}"  id="item_id_{{ $i }}" /><input type="text" class="form-control " placeholder="Defect Name" name="defect_condition[]"
                                value="{{ $record->defect_condition }}" id="defect_condition-{{ $i }}"></td>
                        <td>
                            <textarea class="form-control" rows="3" placeholder="Defect Details" name="defect_name[]"
