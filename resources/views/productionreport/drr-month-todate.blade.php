...
@section('content')
    ...
    <div class="container-fluid">
        ...
        {{ Form::open(['route' => 'filterdailydrr', 'role' => 'form', 'method' => 'post'])}}
        <div class="row">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive sticky-table ">
                        <table class="table table-striped table-bordered" >
                            <thead>
                                   ...
                                   <tr class="sticky-header">
                                    <th  class="sticky-cell" colspan = "{{(count($shops)*3)+1}}" >{{$heading}}</th>
                                ...
                                <tr class="sticky-header" >
                                    <th  class="sticky-cell" rowspan = "2" >Model&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </th>
                                      @foreach ($shops as $item)
                                    <th class="sticky-cell" colspan = "3">{{$item->shop_name}}</th>
                                    @endforeach
                                 ...
                                 <tr  class="sticky-header">
                                    @foreach ($shops as $item)
                                    <th class="sticky-cell" style="background-color: {{$item->color_code}}" >#&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
                                    <th class="sticky-cell" style="background-color: {{$item->color_code}}" >OK UNITS</th>
                                      <th class="sticky-cell" style="background-color: {{$item->color_code}}" >SCORE %</th>
                                    @endforeach
                                    ...
                                    <tr >
                                        ...
                                        @foreach ($shops as $shop)
                                            <td style="background-color: {{$shop->color_code}}">{{$drr_arr[$vehicle->model_id][$vehicle->lot_no][$shop->id]['units']}}</td>
                                               <td style="background-color: {{$shop->color_code}}">{{$drr_arr[$vehicle->model_id][$vehicle->lot_no][$shop->id]['okunits']}}</td>
                        ...
                        <th>  {{ $plant_drr_pershop[$item->id]['units'] }} </th>
                        <th  >{{ $plant_drr_pershop[$item->id]['okunits'] }}</th>
                        <th  >{{ $plant_drr_pershop[$item->id]['score'] }}</th>
                        @endforeach
                        ...
                        <tr class="table-warning">
                            <th><strong>ACTUAL  SCORE</strong></th>
                             @foreach ($shops as $item)
                <th class=" text-center text-white  {{ ($plant_drr_pershop[$item->id]['score'] >= $plant_drr_pershop[$item->id]['drr_target']) ? 'bg-ok' : 'bg-nok'}}" colspan="3"  >{{ $plant_drr_pershop[$item->id]['score'] }}</th>
                ...
                <th class=" text-center" colspan="3" >{{  $plant_drr_pershop[$item->id]['drr_target'] }}</th>
                ...
                @endforeach
                        ...
                        <tr >
                            ...
                            <th  class="text-white {{ ($plant_drr >=  $drr_target_value) ? 'bg-ok' : 'bg-nok'}}"   colspan = "4" >PLANT DRR : <strong>{{  $plant_drr }}</strong></th>

                            <th colspan = "6" >PLANT TARGET : <strong>{{  $drr_target_value }}</strong></th>
